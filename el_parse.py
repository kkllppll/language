...
def failParse(str,tuple):
    ...
    if str == 'getSymb(): неочікуваний кінець програми':
        ...
        exit(6)
    elif str == 'не вказано тип змінної':
        (numLine,lex)=tuple
        print('Parser ERROR: \n\t В рядку {0} не вказано тип змінної {1}.'.format(numLine,lex))
        exit(7)
...
def parseDeclaration():
    ...
    """if (lex, tok) == ('begin', 'keyword'):
        # Якщо зустрінули ключове слово 'begin', завершуємо обробку оголошень
        return False
    """
    
    while (lex, tok) not in [('begin', "keyword")]:
        if parseIdentList():
            parseToken(':', 'decl_op', '\t')
            lexType = parseType()  # Отримуємо тип
            
            parseToken(';', 'punct_op', '\t')

            # Перевірка оголошення змінної та уникнення повторення
            indx = tableOfVar.get(lex)
            if indx is None:
                indx = len(tableOfVar) + 1
                tableOfVar[lex] = (indx, lexType, 'undefined')
                print(f'Оголошено змінну {lex} з типом {lexType}')
            else:
                failParse('повторне оголошення змінної', (numLine, lex, tok))
                return False
            return True
        else:
            return False
...
def printVarTable():
    ...
    for var, (indx, typ, val) in tableOfVar.items():
        print(f"{var}: {typ} (Index: {indx}, Value: {val})")
...
def parseType():
    ...
    if tok == 'keyword' and lex in ('integer', 'real', 'boolean'):
        ...
        failParse('не вказано тип змінної', (numLine, lex))
        return None
...
def getTypeOp(lType, op, rType):
    if lType == 'ident':
        lType = tableOfVar[lexName][2]

    if rType == 'ident':
        rType = tableOfVar[lexName][2]
